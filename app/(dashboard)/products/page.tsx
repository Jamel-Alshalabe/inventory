...
import { api, fmtMoney, type Product } from "@/services/api/api";
...
export default function ProductsPage() {
  ...
  // Laravel paginated response shape: { data: Product[], meta?: { current_page, last_page, total, etc } }
  const productData = response && 'data' in response ? (response.data as Product[]) : [];
  ...
  const parseExcelFile = async (file: File) => {
    try {
      ...
      const existingCodes = new Set(productData.map((p: Product) => p.code.toLowerCase()));
      const parsed = rows
        .map((row) => mapRowToProduct(row))
        .filter((p): p is ProductImportRow => Boolean(p))
        .map((p) => ({
          ...p,
          exists: existingCodes.has(p.code.toLowerCase()),
        }));
      ...
      <Card className="p-4 sm:p-6  border border-slate-700/50 backdrop-blur-xl">
        ...
        <div className="bg-[#111127] border border-gray-700 rounded-xl overflow-hidden shadow-xl">
          ...
          {meta && meta.last_page > 1 && (
            <div className="flex items-center justify-between px-4 py-3 bg-[#0d0d20] border-t border-slate-700">
              <div className="text-xs sm:text-sm text-slate-400">
                عرض {meta.from} إلى {meta.to} من إجمالي {meta.total} منتج
              </div>
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
                ...
                <div className="text-xs sm:text-sm text-white px-2">
